...
insert into tournament(TournamentID,TournamentName,Organizer,PrizePool,StartDate,EndDate,TournamentFormat)
values
(1,'Call of Duty Mobile World Championship',' Activision','1000000','2023-12-15','2023-12-17','Hybrid')
;
...
-- List all teams with their respective captains:
SELECT TeamName, TeamCaptain FROM Team;

-- Show all players along with their gaming handles and the team they belong to:
SELECT PlayerName, GamingHandle, TeamName
FROM Player
JOIN Team ON Player.TeamID = Team.TeamID;

-- Display all tournaments along with their organizers and start dates:
SELECT TournamentName, Organizer, StartDate FROM Tournament;
...
SELECT 
    team.TeamName AS WinnerTeamName
FROM 
    TournamentStandings ts
INNER JOIN 
    Team team ON ts.TeamID = team.TeamID
WHERE 
    ts.Position = 1
    AND ts.TournamentID = 1;
...
FROM 
    TournamentStandings ts
INNER JOIN 
    Tournament t ON ts.TournamentID = t.TournamentID
INNER JOIN 
    Team team ON ts.TeamID = team.TeamID
ORDER BY 
    ts.TournamentID, ts.Position;


-- Retrieve matches with their corresponding match type, winning team, and match date:

SELECT    -- both team names
    bm.MatchID,
    m.MatchType,
    bm.WinnerID,
    m.MatchDate,
...
FROM 
    BracketMatch bm
LEFT JOIN 
    Matches m ON bm.MatchID = m.MatchID
LEFT JOIN 
    Team t1 ON m.Team1ID = t1.TeamID
LEFT JOIN 
    Team t2 ON m.Team2ID = t2.TeamID;

SELECT -- winnerteamname
    bm.MatchID,
    m.MatchType,
    t_winner.TeamName AS WinnerTeamName,
    m.MatchDate
FROM 
    BracketMatch bm
LEFT JOIN 
    Matches m ON bm.MatchID = m.MatchID
LEFT JOIN 
    Team t_winner ON bm.WinnerID = t_winner.TeamID;

SELECT  -- combo of above two 
    bm.MatchID,
    m.MatchType,
    t_winner.TeamName AS WinnerTeamName,
    m.MatchDate,
...
-- Retrieve all information about matches, including team names, tournament details, and venue information:
SELECT m.MatchID, m.MatchType, m.Team1Score, m.Team2Score, m.Round, m.MatchDate,
       t1.TeamName AS Team1Name, t2.TeamName AS Team2Name,
       t.TournamentName, t.Organizer, t.PrizePool, t.StartDate, t.EndDate,
       v.VenueName, v.Location, v.Capacity, v.Amenities, v.BookingAvailability, v.RentalCost
FROM Matches m
...
-- Retrieve information about players, including their team details and nationality:
SELECT p.PlayerID, p.PlayerRosterName, p.PlayerName, p.Age, p.Nationality, p.GamingHandle,
       t.TeamName, t.TeamCaptain, t.DateOfFormation, t.SponsorshipDetails, t.TeamRegion, t.TeamMascot
FROM Player p
JOIN Team t ON p.TeamID = t.TeamID;

-- Retrieve details about teams with their coaches
SELECT t.TeamID, t.TeamName, t.TeamCaptain, t.DateOfFormation, t.SponsorshipDetails, t.TeamRegion, t.TeamMascot,
       c.CoachName, c.CoachingExperience, c.CoachRole
FROM Team t
JOIN Coach c ON t.TeamID = c.TeamID;

-- teams and their positions in tournament
SELECT
    t.TeamName,
    RANK() OVER (ORDER BY COALESCE(SUM(CASE WHEN m.Team1ID = t.TeamID THEN (CASE WHEN m.Team1Score > m.Team2Score THEN 1 ELSE 0 END) ELSE (CASE WHEN m.Team2Score > m.Team1Score THEN 1 ELSE 0 END) END), 0) DESC) AS Position
FROM
    Team t
LEFT JOIN
    Matches m ON (t.TeamID = m.Team1ID OR t.TeamID = m.Team2ID) AND m.TournamentID = 1
GROUP BY
    t.TeamID
ORDER BY
    Position;

-- team their positions and amount they won
SELECT
    t.TeamName,
    positions.Position,
    COALESCE(pp.PrizeAmount, 0) AS PrizeAmount
FROM
    Team t
LEFT JOIN
    (SELECT
    ...
    GROUP BY
        t.TeamID) positions
ON
    t.TeamID = positions.TeamID
LEFT JOIN
    PrizePool pp ON pp.TournamentID = 1 AND pp.Place = positions.Position
ORDER BY
    positions.Position;
...
LEFT JOIN
    PrizePool pp ON pp.TournamentID = 1 AND pp.Place = t.Position
ORDER BY
    Position;
...
-- positions of teams and the prizes they won
SELECT
    ts.TournamentID,
    ts.TeamID,
    ts.Position AS TeamPosition,
    IFNULL(pp.PrizeAmount, 0) AS PrizeAmount -- COALESCE is one of alternative
FROM
    TournamentStandings ts
LEFT JOIN
    PrizePool pp ON pp.TournamentID = ts.TournamentID AND pp.Place = ts.Position
ORDER BY
    ts.TournamentID, ts.Position;
-- with team names
SELECT
    ...
    t.TeamName,
    ts.Position AS TeamPosition,
    IFNULL(pp.PrizeAmount, 0) AS PrizeAmount
FROM
    TournamentStandings ts
LEFT JOIN
    Team t ON t.TeamID = ts.TeamID
LEFT JOIN
    PrizePool pp ON pp.TournamentID = ts.TournamentID AND pp.Place = ts.Position
ORDER BY
    ts.TournamentID, ts.Position;
